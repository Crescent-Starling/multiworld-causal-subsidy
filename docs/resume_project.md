...
- **设计 LLM-driven Agent 扩展接口**：实现 `LLMClient`（支持 OpenAI / Anthropic 双后端切换）与 `LLMSubsidyAgent`（含 Chain-of-Thought 提示词工程），并内置 Mock 回退模式保证无 API 环境下的可复现测试；该模块作为规则型/认知型 Agent 的补充扩展，接口已打通，真实 LLM 调用为可选能力
...
- **Designed extensible LLM-driven Agent interface**: implemented `LLMClient` (OpenAI / Anthropic dual-backend switch) and `LLMSubsidyAgent` (with Chain-of-Thought prompt engineering), with built-in Mock fallback ensuring reproducible testing without API access; this module serves as an optional extension to rule-based / cognitive agents, with interfaces fully wired and real LLM calls as an optional capability
...
| Agent模型 | 前景理论+心理账户(4类型+迁移)+有限理性+疲劳脱敏（规则型/认知型）；LLM Agent（接口已实现，Mock回退，真实LLM调用为可选） |
