...
实验表明，认知型策略在ROI上较随机基线提升18.6倍，多世界对比确认策略效应统计显著，社会效应估计揭示BA网络正向溢出与WS网络抑制效应的质性差异。CATE驱动策略通过正uplift过滤与效率优先面额调整，验证了因果模型uplift排序与仿真层行为的一致性。本框架为平台补贴策略的离线因果评估提供了一套可复制、可解释的技术方案，为后续的在线实验与策略部署奠定了方法论基础。
